...
export function usePoolTokenBalances(): { [token: string]: BigNumber } | null {
  ...
  usePoller((): void => {
    async function pollBalances(): Promise<void> {
      ...
      let balances: { [p: string]: any }
      let tokenBalances: BigNumber[] = []
      let ethBalance: BigNumber = Zero

      if (chainId == ChainId.OPTIMISM_KOVAN) {
        ...
        tokenBalances = await ethcallProvider.all(balanceCalls, {})
        ethBalance = await library.getBalance(account)
